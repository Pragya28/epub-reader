...
- **Series are read-only and system-detected** — parsed from Calibre's `<meta name="calibre:series">`/`calibre:series_index">` OPF metadata or, failing that, EPUB3 `belongs-to-collection` entries typed `series` (`opf-parser.ts`; Calibre wins when both are present, `set`-typed collections are ignored) and upserted via `upsertSeriesMembership()` on import, matching an existing series case-insensitively by name or creating one. There is no `renameSeries`/`deleteSeries` action — `isCollection(grouping)` is the one guard that keeps a caller from mistaking a series id for a collection's.
