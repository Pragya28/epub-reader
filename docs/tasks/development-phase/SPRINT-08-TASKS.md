...
- Day 6 item 31 (backup/export workflow validation) ✅ — the backup/restore feature is built (`src/services/backup/` + `features/library/actions/{export-library,import-backup,reset-library}.ts`, Settings UI, `e2e/backup-restore.spec.ts`). Item 28 (full-chain workflow test) ✅ — already covered by `e2e/cross-device.spec.ts` (Day 5, #18). Item 30 (release checklist) ✅ — `RELEASE_CHECKLIST.md`. Item 29 (long-session stress) ✅ — `use-reader-engine.test.ts` gains a bounded-listeners/blob-URL regression block, plus `backup.perf.test.ts` for real-EPUB backup/search stress at library scale. Item 27 (manual regression pass) ✅ — live browser walkthrough (import, read, search, organize, backup/restore, cross-tab, dark/mobile) found and fixed three real bugs: a kebab-case SVG attribute React console error in `word-mark.tsx`, five nav buttons losing native link semantics, and a stale "Space used" readout after "Delete all". **Day 6 fully complete.**
- Day 7 ✅ complete — `CHANGELOG.md` added, `README.md` refreshed, versioning process established (version shown in-app), a real production-build (built + served, not just compiled) smoke test added, and a final-cleanup pass removed the dead pre-project `poc/` directory (19 files) plus an unused dependency and two minor lint findings. **Sprint 8 fully complete, all 7 days.**
...
- **P2 — main JS bundle 945.95 kB (295.92 kB gzip)** — out of scope for Day 1; already tracked as Day 3 items 9 and 13. **Confirmed resolved**: the 2026-08-30 full-codebase audit rerun (`docs/reports/development-phase/AUDIT_REPORT.md`, `docs/reports/development-phase/code-reviews/code-review-2026-08-30.md`) explicitly notes "the prior audit's bundle-size P2 was already resolved by Sprint 8 Day 3's route splitting" — score now 20/20, 0 P0-P3 outstanding.
...
33. ✅ **Final cleanup** — a full-repo `eslint .` + `depcheck` pass (not scoped to items 34/35's own diffs) surfaced real, unrelated debt: the entire `poc/` directory (19 files) — pre-project proof-of-concept scaffolding from before Sprint 1, unreferenced anywhere in `src/`, already flagged as known-stale lint noise as far back as Sprint 5's own regression note — deleted outright; `@fontsource-variable/jetbrains-mono` was a listed dependency whose font files were never actually imported anywhere (`index.css` only references the font-family name as a fallback string) — removed; two unused-var lint warnings in test files (dead `docAfter`/`local` bindings) cleaned up; and a real (if minor) `use-reader-screen.ts` memo bug — `toc` fell back to a fresh `[]` literal every render when `parsedBook` was absent, silently defeating `currentChapterLabel`'s `useMemo` below it — fixed by memoizing `toc` itself. `eslint .` warning count: 13 → down to only the pre-existing, accepted shadcn Fast-Refresh notes and untracked local artifacts (`coverage/`, gitignored `src/tests/dev/`). Verified: full Vitest suite (748/748) and `pnpm build` both green after every change.
34. ✅ **Versioning** — `package.json`'s `version` is now the single source of truth end to end: injected into the build via `vite.config.ts`'s `__APP_VERSION__` define (read from `package.json` at config-load time, no duplicated version string anywhere), typed in `vite-env.d.ts`, and shown to users at the bottom of Settings (`Librune v1.0.0`, muted). `RELEASE_CHECKLIST.md`'s Versioning section updated with the actual process: bump `package.json`, move `CHANGELOG.md`'s `[Unreleased]` entries under a dated heading, bump `BACKUP_VERSION` separately if the archive format changed, tag `vX.Y.Z`.
35. ✅ **Production build validation** — `pnpm build` already runs on every pre-push hook (tsc -b + vite build + bundle-size guard), so the build itself was already continuously validated; what was missing was exercising the actual built output. Added `e2e/production-build.spec.ts` + `playwright.build.config.ts` (run via `pnpm test:e2e:build`): builds, serves the real `dist/` output through `vite preview`, confirms the service worker actually activates (the regular e2e suite runs against the dev server, which never registers one — `devOptions.enabled: false`), and imports a book end to end. Filters two expected-not-bug console/network signals (the reader iframe's deliberate `sandbox="allow-same-origin"` script-block warning, and `@vercel/speed-insights`'s script 404 outside real Vercel infra) rather than asserting a blindly-empty error list. A release-time check (documented in `RELEASE_CHECKLIST.md`), not wired into the per-push CI job — a full build+preview cycle is too slow for that.

### Done Criteria

✅ Complete. All of Day 7 (items 32-35) done — Sprint 8 fully complete.
