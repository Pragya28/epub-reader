...
35. ✅ **Production build validation** — `pnpm build` already runs on every pre-push hook (tsc -b + vite build + bundle-size guard), so the build itself was already continuously validated; what was missing was exercising the actual built output. Added `e2e/production-build.spec.ts` + `playwright.build.config.ts` (run via `pnpm test:e2e:build`): builds, serves the real `dist/` output through `vite preview`, confirms the service worker actually activates (the regular e2e suite runs against the dev server, which never registers one — `devOptions.enabled: false`), and imports a book end to end. Filters two expected-not-bug console/network signals (the reader iframe's deliberate `sandbox="allow-same-origin"` script-block warning, and `@vercel/speed-insights`'s script 404 outside real Vercel infra) rather than asserting a blindly-empty error list. A release-time check (documented in `docs/RELEASE_CHECKLIST.md`), not wired into the per-push CI job — a full build+preview cycle is too slow for that.
