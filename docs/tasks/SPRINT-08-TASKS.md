...
- Day 6 item 31 (backup/export workflow validation) ✅ — the backup/restore feature is built (`src/services/backup/` + `features/library/actions/{export-library,import-backup,reset-library}.ts`, Settings UI, `e2e/backup-restore.spec.ts`). Item 28 (full-chain workflow test) ✅ — already covered by `e2e/cross-device.spec.ts` (Day 5, #18). Items 27, 29, 30 (regression pass, long-session stress, release checklist) still open.
...
🟡 Items 28, 30, 31 done — the full-chain e2e test, release checklist, and backup/export feature are all built. Items 27 and 29 remain: a manual/exploratory regression pass and long-session stress coverage.
