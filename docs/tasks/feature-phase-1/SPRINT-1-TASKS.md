...
8. ❌ **Migration runner** — `backend/db/migrate.ts`: applies pending numbered `.sql` files in order, forward-only. Records each in `schema_migrations` (filename, SHA-256, applied_at) atomically with the file; fails if an already-applied file's hash changed; takes a transaction-scoped advisory lock (`pg_advisory_xact_lock`) so overlapping runs cannot double-apply, and connects over the direct (unpooled) connection string, since migrations need interactive transactions. `pnpm db:migrate` applies; `pnpm db:status` lists applied and pending. Tests: a second run is a no-op; an edited applied file is rejected; status lists pending files; queries under test run on `pglite` with the migrations applied, so a query against a column no migration creates fails.
