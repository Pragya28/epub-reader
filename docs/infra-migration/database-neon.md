...
**What.** The sync backend's relational database is a Postgres database hosted on Neon, provisioned through the Vercel Marketplace integration. It holds four small tables — `users`, `sync_state` (per user: hash of the registration proof, last successful sync), `invites` (hashed single-use invite codes) and `devices` (one row per device, holding that device's token hash, previous token hash, rotation and last-seen timestamps) — for one owner plus any users they invite. It never holds library content: the encrypted archive lives in blob storage, and the encryption key never reaches the server.

**Why this one.**

- A free tier is a hard requirement for this personal-use app. Neon's free plan comfortably covers the workload (one owner plus perhaps a few invited users, a few devices each, a handful of rows, manual sync).
...
| Atomicity guarantees     | Single-statement queries (`INSERT … ON CONFLICT`, conditional `UPDATE … RETURNING`, CTEs) plus DB constraints | The first-user `setup` guard, single-use invite consumption, and rotation/recovery races are enforced by the database, not by application code.                                                                                                                                                                              |
...
3. **Apply the schema** by running `backend/db/migrations/*.sql` against it. If the destination is Postgres-compatible this should work unchanged; if it is not, translate the tables (types for uuid/timestamp/text) and re-verify every constraint the guarantees above depend on.
...
6. **Carry the data across.** The tables hold a handful of rows. For a Postgres destination: dump from the old database (`pg_dump`) and restore into the new one. Because token hashes and device rows come with it, existing devices keep working without noticing the move.
