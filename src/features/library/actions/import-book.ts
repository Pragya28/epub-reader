...
export async function importBook(file: File) {
  ...
  // Likewise it never touches the library-level `isLoading` flag: that swaps
  // the grid for "Loading your library…", which during a bulk import kept the
  // grid blank for the whole batch. The FAB shows its own spinner instead.
  ...
  libraryStore.getState().addBook(book);
